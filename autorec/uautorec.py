...
"""
Implementation of UAutoRec (user-based autorec).
The author proved that IAutoRec (movie-user matrix) is better than UAutoRec (user-movie matrix). I will implement IAutoRec later.

For UAutoRec, the rmse of the original paper is about 0.867.
However, my rmse is just about 0.9286 on the whole dataset.  
...
class IAutoRec:
    ...
    def train_generator(self, Xtrain, batch_sz):
        ...
        while True:  # loop indefinitely. important!
            ...
            #Xtrain = shuffle(Xtrain)  # affect rows
    ...
    def fit(self, Xtrain, Xtest, batch_sz=128, epoch=50):
